...
def get_train(name, seed=3, workers=0, config=None):
    if name == 'colon_cancer':
        ...
        ds = MitoPreprocessedBagsCross(path="data/mito_scale_patches_800", train=True, shuffle_bag=True, data_augmentation=True,
                                   fold_id=config.fold_id, folds=config.folds, random_state=seed)
        ds_push = MitoPreprocessedBagsCross(path="data/mito_scale_patches_800", train=True, push=True, shuffle_bag=True,
                                        fold_id=config.fold_id, folds=config.folds, random_state=seed)
    elif name == 'mnist':
...
def get_valid(name, seed=3, workers=0, config=None):
    if name == 'colon_cancer':
        ...
        ds_valid = MitoPreprocessedBagsCross(path="data/mito_scale_patches_800", train=False, all_labels=True, fold_id=config.fold_id,
                                         folds=config.folds, random_state=seed)
        ds_push_valid = MitoPreprocessedBagsCross(path="data/mito_scale_patches_800", train=False, all_labels=True, fold_id=config.fold_id,
                                              folds=config.folds, random_state=seed, push=True, )
...
def get_test(name, seed=3, workers=0, config=None):
    if name == 'colon_cancer':
        ...
        ds_test = MitoPreprocessedBagsCross(path="data/mito_scale_patches_800", train=False, test=True, all_labels=True,
                                        fold_id=config.fold_id, folds=config.folds, random_state=seed)
        ds_push_test = MitoPreprocessedBagsCross(path="data/mito_scale_patches_800", train=False, test=True, push=True, all_labels=True,
                                             fold_id=config.fold_id, folds=config.folds, random_state=seed)
    elif name == 'mnist':
